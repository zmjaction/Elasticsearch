...
class SearchView(View):
    ...
    def get(self, request):
        ...
        # 通用部分
        # 实现搜索关键词keyword加1操作
        # 主要实现的是热门搜索,这个是在result.html页面中
        redis_cli.zincrby("search_keywords_set", key_words)  # # redis> ZINCRBY salary 2000 tom   # tom 加薪啦！"40000"
        # 获取topn个搜索词
        topn_search_clean = []
        ...
        for hit in response["hits"]["hits"]:
            ...
            try:
                ...
                if "NewsContent" in hit["highlight"]:
                    hit_dict["NewsContent"] = "".join(hit["highlight"]["NewsContent"])
                else:
                    hit_dict["NewsContent"] = hit["_source"]["NewsContent"][:10] # 如果显示过长 页面会没有空间
        ...
        return render(request, "result.html", {"page": page,
                                               "all_hits": hit_list,  # 处理之后的数据
                                               "key_words": key_words, # 搜索关键词,显示到搜索栏
